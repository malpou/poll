...
- `specs/<area>/spec.md` is the source of truth for behavior. Format is
  OpenSpec: `### Requirement:` (SHALL prose) + `#### Scenario:`
...
- ANY behavior change updates the matching `spec.md` AND its colocated
  `specs/<area>/*.spec.ts` in the SAME change. New behavior area = new
...
- `specs/DESIGN.md` is the source of truth for look and feel (palette, type,
...
- Unit tests are `src/**/*.test.ts` (vitest). Never name them `.spec.ts` —
  Playwright globs `specs/**/*.spec.ts`; helpers under `specs/support/`
  stay plain `.ts`.
- Design tokens and motion rules: `specs/DESIGN.md`. Product, data model,
  routes, conventions: `specs/PROJECT.md`.
