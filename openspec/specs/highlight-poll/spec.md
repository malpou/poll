# highlight-poll Specification

## Purpose

A text-option poll type answered by spending a budget: the organizer
supplies two or more text options and a fixed marker-stroke budget, and each
invitee distributes strokes across the options by tapping (cumulative
voting). Results total the strokes per option. Option management, closing,
and reopening reuse the question type's behavior
(openspec/specs/question-options, poll-closing).
