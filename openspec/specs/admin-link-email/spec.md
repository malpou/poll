# admin-link-email Specification

## Purpose

An optional email field on the create form so the organizer gets a durable copy
of their capability link, plus a second admin secret (a short code) that gates
the organizer pages for polls created with an email — so a leaked URL alone no
longer grants admin access. Polls created without an email are unchanged.
...
creation. Because the organizer already has a durable copy of the link, the
organizer dashboard SHALL NOT show its "save this link" reminder for a poll
created with an email; a poll created without one SHALL still show it.
...
#### Scenario: Emailed poll hides the save-your-link reminder

- GIVEN a poll created with an organizer email
- WHEN the organizer views the dashboard
- THEN the "save this link" reminder is not shown
- AND a poll created without an email still shows it
